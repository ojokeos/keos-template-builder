...
const workflowStatus = computed(() => (campaign.value as any).workflow_status ?? 'draft');
...
const bodyError = computed(() => {
  const e = blockingErrors.value.find((x) => x.message === 'Message body is required');
  if (e) return e.message;
  if (bodyCount.value > bodyLimit.value) return `Body exceeds ${bodyLimit} characters for ${selectedPlatform.value}.`;
  return undefined;
});
  ...
  <div class="keos-notification-builder">
    ...
    <div class="kb-push-layout">
          ...
          <div class="kb-push-form-head">
            ...
            <div class="kb-push-form-head-row">
              ...
              >
                ...
                <option v-for="p in PUSH_PRESETS" :key="p.id" :value="p.id">{{ p.label }}</option>
              </select>
            </div>
          </div>
          <SectionMessage
      ...
      <main class="kb-push-canvas">
        ...
        <div class="kb-push-preview-chrome">
          ...
          <div class="kb-push-device-toggle" role="tablist" aria-label="Preview platform">
            <button
          ...
          <div class="kb-push-preview-frame">
            ...
            <PreviewPanel
              ...
              @update:selected-platform="(p: Platform) => (selectedPlatform = p)"
            />
          </div>
        </div>
      </main>
    ...
    <div v-if="presetConfirmOpen" class="kb-confirm-overlay" role="dialog" aria-modal="true" aria-labelledby="preset-confirm-title">
      <div class="kb-confirm-dialog">
        <h2 id="preset-confirm-title" class="kb-confirm-title">Replace content?</h2>
        <p class="kb-confirm-text">Current changes will be replaced by the preset. Continue?</p>
        <div class="kb-confirm-actions">
          <button type="button" class="kb-push-action kb-push-action--secondary" @click="presetConfirmOpen = false; pendingPreset = null">Cancel</button>
          <button type="button" class="kb-push-action kb-push-action--primary" @click="pendingPreset && applyPreset(pendingPreset)">Replace</button>
        </div>
      </div>
    </div>
