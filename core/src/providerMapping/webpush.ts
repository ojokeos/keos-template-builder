 ...
 */
export function toWebPush(campaign: Campaign): ProviderMappingResult {
  ...
  if (Array.isArray((message as any).actions) && (message as any).actions.length > 0) {
    const actions = (message as any).actions as Array<{ id: string; label: string }>;
    payload.actions = actions.map((a) => ({
      action: a.id,
      title: a.label,
    }));
    (payload.data as Record<string, unknown>).actions = actions;
  }
